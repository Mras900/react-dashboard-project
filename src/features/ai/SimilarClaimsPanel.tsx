...
export function SimilarClaimsPanel() {
  ...
  return (
    <section className="cc-ai-panel rounded-lg border p-4">
      <div className="mb-4">
        <p className="text-xs font-black uppercase tracking-wide text-[#7A90A8]">IA / reclamos parecidos</p>
        <h2 className="cc-ai-title text-xl font-black">Buscar casos similares</h2>
      </div>
      <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_160px_140px_120px_120px]">
        <input className="cc-ai-input h-10 rounded-lg border px-3 text-sm font-semibold outline-none" onChange={(event) => setQuery(event.target.value)} placeholder="Describe caso, ticket, observacion o comuna" value={query} />
        <select className="cc-ai-input h-10 rounded-lg border px-3 text-sm font-bold outline-none" onChange={(event) => setTerritorio(event.target.value as 'rm' | 'regiones' | 'nacional')} value={territorio}>
          <option value="rm">RM</option><option value="regiones">Regiones</option><option value="nacional">Nacional</option>
        </select>
        <input className="cc-ai-input h-10 rounded-lg border px-3 text-sm font-semibold outline-none" onChange={(event) => setComuna(event.target.value)} placeholder="Comuna" value={comuna} />
        <input className="cc-ai-input h-10 rounded-lg border px-3 text-sm font-semibold outline-none" onChange={(event) => setYear(Number(event.target.value))} type="number" value={year} />
        <input className="cc-ai-input h-10 rounded-lg border px-3 text-sm font-semibold outline-none" max={12} min={1} onChange={(event) => setMonth(Number(event.target.value))} type="number" value={month} />
      </div>
      <div className="cc-ai-muted mt-3 flex flex-wrap gap-3 text-sm font-bold">
        ...
        <select className="cc-ai-input h-9 rounded-lg border px-3 text-sm font-bold outline-none" onChange={(event) => setRiskLevel(event.target.value as '' | 'alto' | 'medio' | 'bajo')} value={riskLevel}>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        ...
        <button className="cc-ai-secondary-button inline-flex items-center gap-2 rounded-lg border px-4 py-2 text-sm font-black disabled:opacity-50" disabled={loading || !query.trim()} onClick={runSummary} type="button"><Sparkles size={16} /> Resumir coincidencias con IA</button>
      </div>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        {items.map((item, index) => {
          ...
          return (
            <article className="cc-ai-result-card rounded-lg border p-3" key={`${item.ticket ?? index}`}>
              ...
              <h3 className="cc-ai-title mt-2 text-sm font-black">Ticket {String(item.ticket ?? 'sin ticket')}</h3>
              <p className="cc-ai-muted mt-1 text-xs font-semibold">{String(item.estado ?? 'Sin estado')} · {String(item.prioridad ?? 'Sin prioridad')}</p>
              <p className="cc-ai-text mt-2 text-sm font-semibold">{String(item.observacion_resumida ?? 'Sin observacion')}</p>
