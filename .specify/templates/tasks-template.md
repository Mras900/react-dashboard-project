...
- [ ] T004 Identify delivery stage: importacion, cruce censo-reclamos, dashboard territorial, despliegue
- [ ] T005 Document existing behavior that must not regress
...
- [ ] T010 Configure error handling and logging infrastructure, including non-blank UI failure states
- [ ] T011 Setup environment configuration management
- [ ] T012 Confirm `data/raw` remains untouched and heavy Censo/ZIP/Parquet artifacts are ignored
...
- [ ] TXXX Run `npm run build` for frontend changes
- [ ] TXXX Validate `/api/health`, `/api/dashboard/resumen`, and `/api/dashboard/comunas` for backend changes
- [ ] TXXX Verify CSV/XLSX import if import or data ingestion changed
- [ ] TXXX Verify Leaflet layers, RM view, Regiones view, Ruta Visitador, and filters if territorial behavior changed
...
- [ ] TXXX Document modified files, commands run, validation results, and detected risks
